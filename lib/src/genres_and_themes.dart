...
// ignore_for_file: camel_case_types, non_constant_identifier_names, use_build_context_synchronously, must_be_immutable
...
class genres_and_themes extends StatefulWidget {
  genres_and_themes({super.key, required this.picked_genre});
  late dynamic picked_genre ; 
 

  @override
  State<genres_and_themes> createState() => _genres_and_themesState();
}



class _genres_and_themesState extends State<genres_and_themes>  {

    // current_time in unix / epoch 
    ...
    dynamic getData() async {
      ...
      if ( widget.picked_genre == 'Third_Person') // Third Person 
      {    
         query_term = 'Third Person';
         query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ; where player_perspectives = 2 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc ; limit 100 ;';
      }
      else if ( widget.picked_genre == 'Action') // Action 
      {
        query_term = 'Action';
        query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ;  where themes = 1 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc; limit 100 ;';
      }
      else if ( widget.picked_genre == 'Fighting' ) // Fighting 
      {
        query_term = 'Fighting';
        query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ;  where genres = 4 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc; limit 100 ;';

      }
      else if ( widget.picked_genre == 'Shooter' ) // Shooter 
      {
        query_term = 'Shooter';
        query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ;  where genres = 5 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc; limit 100 ;';

      }
      else if ( widget.picked_genre == 'Open_World' ) // Open_World 
      {
        query_term = 'Open World';
        query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ; where themes = 38 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc; limit 100 ;';

      }
       else if ( widget.picked_genre == 'Simulator' ) // Simulator 
      {
        query_term = 'Simulator';
        query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ; where genres = 13 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc; limit 100 ;';

      }
      else if ( widget.picked_genre == 'Racing' ) // Racing 
      {
        query_term = 'Racing';
        query = 'f name, cover, cover.url, summary, storyline, first_release_date, platforms.name, total_rating, screenshots.image_id, genres.name ; where genres = 10 & first_release_date < $unix_time_now & cover != null & version_parent = null & total_rating != null ; sort first_release_date desc; limit 100 ;';
  ...
  Widget build(BuildContext context)   {
    ...
    return Scaffold(
     ...
     appBar: AppBar(
        ...
        titleTextStyle: const TextStyle(color:  Color(0xffe07a5f), fontSize: 22 , fontFamily: 'Mulish-Bold',), 

        foregroundColor: const Color(0xffe07a5f),
        backgroundColor: const Color(0xffffffff),
        title:   Text(query_term) ,
      ),
